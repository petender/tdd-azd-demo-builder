...
Azure Demo Builder automates the end-to-end lifecycle of Azure demo environments. Describe the Azure scenario you want to build in natural language, and a pipeline of specialized AI agents handles requirements gathering, architecture design, Bicep code generation, deployment, and demo guide creation — all within VS Code.
...
- **Governance-aware**: Before generating any Bicep code, the agent discovers active Azure Policy assignments (including management group-inherited policies) and maps every `Deny` policy to compliant parameter values.
